...
const inspectedElementCache = new LRU({
  max: 25,
});

export function inspectElement({bridge, element, path, rendererID}) {
  const {id} = element;
    ...
    switch (type) {
      ...
      case 'hydrated-path':
        ...
        if (inspectedElement !== null) {
          ...
          // Merge hydrated data
          fillInPath(inspectedElement, value, path, hydrateHelper(value, path));
