...
function SettingsContextController({
  ...
  const [parseHookNames, setParseHookNames] = useLocalStorage(
    LOCAL_STORAGE_PARSE_HOOK_NAMES_KEY,
    false,
  );
  const [hideConsoleLogsInStrictMode, sethideConsoleLogsInStrictMode] =
    useLocalStorage(LOCAL_STORAGE_HIDE_CONSOLE_LOGS_IN_STRICT_MODE, false);
  const [showInlineWarningsAndErrors, setShowInlineWarningsAndErrors] =
    useLocalStorage(LOCAL_STORAGE_SHOW_INLINE_WARNINGS_AND_ERRORS_KEY, true);
